...
> **`sennel workflow` は experimental である。** 実装は安定しているが運用方法論は未確定で、usage patterns may change（使い方は今後変わる可能性がある）。昇格条件は `src/workflow/AGENTS.md` を参照。

## プロジェクトルール

### 開発ワークフロー

- `src/skills/` のスキルソースや `src/presets/` のテンプレートを変更した場合は `sennel upgrade` を実行して、プロジェクトのスキル・設定に反映すること。
- `sennel upgrade` はスキル（`.claude/skills/`, `.agents/skills/`）やテンプレートの差分を検出し、変更があったファイルのみ更新する。
