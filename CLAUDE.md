...
- スキルが利用できない環境では、`senti flow set init --request "<要望>"` で preparing flow を作成すること。返却された runId を以後の `senti flow prepare ... --run-id <runId>` と `senti flow run dispatch --expect-run-id <runId>` に必ず指定し、詳細は各コマンドの `--help` を参照すること。

### Worktree の境界を越えない（MUST）

`flow prepare --worktree` で作成した worktree で作業している間、以下を厳守する:

- **MUST: worktree パス外に `cd` してはならない。** 唯一の正当な離脱は `senti flow run finalize-cleanup` の cleanup 完了後（finalize skill がその遷移を明示的に案内する）のみ。
...
作業開始前に `senti check freshness` を実行すること。
結果が `stale` または `never-built` の場合は、`senti docs build` の実行をユーザーに提案すること。`indeterminate` の場合は制限内容を伝え、docs が最新であるとは扱わないこと。

### 開発ワークフロー

- `src/skills/` または `src/presets/` のスキル・プリセット成果物を変更した場合は `senti upgrade` を実行して、プロジェクトのスキル、AGENTS/CLAUDE の SDD セクション、および preset copy を更新すること。
- docs 用テンプレートの変更を生成済み docs に反映する場合は `senti docs build` を実行すること。
