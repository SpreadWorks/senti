...
| src | 12 | check command entrypoint, docs command entrypoint, CLI dispatcher, Render localized help for core and plugin commands., hook command bootstrap, Metrics command entrypoint, cli, CLI entrypoint for preset inspection and tree rendering., Top-level CLI router and command bootstrapper, interactive setup wizard and bootstrap command, spec command namespace entrypoint |
