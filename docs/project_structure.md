...
| --- | --- | --- |
| src/flow | 70 | flow merge orchestrator, flow reporting command, Workflow definition and state-derivation core, flow command registry and lifecycle control, spec rationale section builder for prompts, lib, Draft lifecycle domain model and validator, flow-context, Gate phase resolver, controller, Context search and retrieval engine, get guardrail flow command, cli, Prompt retrieval command, Spec view renderer, library, Auto-check input resolver, Resolved flow context builder, Auto-check evaluator, Finalize cleanup and rescue orchestrator, Finalize commit step command, finalize merge command wrapper, flow-command, artifact trust and validation layer, test-header-validator, regression command orchestration |
...
| src/lib | 38 | lib, Metric dimension normalizer shared by agent runner and metrics aggregator, agent execution engine, CLI utility foundation, route, model, configuration loader and validator, Validation constants, dependency injection and runtime composition, command dispatcher and runtime logging, flow state helper library, flow coordination service, flow persistence and migration layer, git integration utilities, guardrail loading, merging, and filtering, lint-executor, middleware, Makefile test-target parser, view, subprocess execution core, Fluent builder for AI prompts assembled from labeled sections, Provider plug-ins: per-CLI argument flags and stdout parsers, rules validation and rendering, skill deployment pipeline, spec-json-io, test command source selection, shared type contract placeholder |
| src | 10 | cli, Flow CLI entrypoint and dispatcher, main CLI bootstrap and router, setup command orchestrator |
