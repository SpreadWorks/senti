...
| --- | --- | --- |
| src/flow | 82 | command, Report generation and formatting for flow status artifacts, Defines the `flow review` command and its multi-phase review execution, parsing, and artifact persistence., Provides the canonical workflow definition, lifecycle model, and step-resolution utilities for SDD flow execution., spec rationale section builder for prompts, lib, Abstract base command for flow subcommands, draft-lifecycle-model, routing, Flow context resolver, contract, Translates gate retry state into a normalized display-oriented recovery phase object., Provides gate step and phase translation utilities used by recovery and state inspection logic., Defines a preflight check command for flow prerequisites, repository state, and GitHub tool readiness., flow-context-search, get guardrail flow command, cli, Computes the next actionable workflow instruction from flow state, rules, schemas, and recovery context., flow-prompt-loader, Runtime log retrieval command, status reporter, step-instructions-loader, Review evidence domain model and normalization layer, spec-view-renderer, library, auto-check input resolver, context envelope resolver, artifact-reader, Provides the internal retry-recovery engine for SDD flow operations, including validation, evidence comparison, persistence, and eligibility decisions for retry resets., failure-classifier, auto-check command orchestrator, flow task completion command, regression-runner, finalize cleanup runner, Finalize commit command adapter, finalize merge command wrapper, controller, Finalize workflow safety and Git-state coordination, gate orchestrator, implementation confirmation command, spec bootstrapper, draft reset handler, Read-only report retrieval command for finalized flow output, Flow command adapter for report generation, flow-command, retrospective synthesis command, Scenario validity test runner, Test execution command, test artifact review command, Flow command for safely applying overview deltas to spec artifacts., Flow command for managing `user_approval` state in spec metadata., Flow command that toggles automatic approval behavior with guard checks., broad mode setter, Issue log persistence and mutation command, Issue setter command handler, Metric setter command handler, Note setter command handler, requirement status setter, flow subcommand handler, task scope decision engine, Artifact schema, validation, trust, and lifecycle management, test-header-validator, Flow regression test planner and subprocess execution layer |
| src/lib | 47 | lib, agent default configuration provider, Metric dimension normalizer shared by agent runner and metrics aggregator, Foundational CLI/path helper layer shared across commands., Registers and organizes all command namespaces for the CLI dispatcher., model, Loads and validates project configuration while exposing canonical config and output paths., Shared domain constants for flow validation and control., Service-locator container bootstrapping config, agent, providers, paths, and scanner utilities for all CLI subcommands, CLI command dispatch and envelope/error runtime handling, Error preview formatter utility, Path translator that maps worktree-internal artifacts to durable main-repo locations for finalize cleanup, Option normalization utility, Git and GitHub command orchestration with normalized repository-state introspection., Core utility for translating glob patterns into executable regular expressions., guardrail loading, merging, and filtering, Supplies the internal runtime for declarative flow hooks and their command execution., Safe include preprocessor for templated documentation and skill content., lint-executor, middleware, Makefile parsing utility for automated test command discovery., view, Preset asset deployment utility, Core process runner abstraction and command error normalization., Fluent builder for AI prompts assembled from labeled sections, Agent provider adapter layer, Runtime execution logging framework, Skill deployment and synchronization utility for local agent environments., spec-json-io, Canonical step identifier migration layer for flow and task state., Test command source modeling and priority-based selector., Shared type-related library module; implementation not shown in the extract. |
