...
このドキュメントは、sdd-forge のプリセットを **ビルトイン (`src/presets/<key>/`)** または **プロジェクトローカル (`.sdd-forge/presets/<key>/`)** として新規作成する際の手順書である。spec 191 で導入された DI (Dependency Injection) コンテナ契約に基づいて記述しており、AI エージェントが本ドキュメントだけを読んでプリセットを組み立てられるように、仕様・手順・落とし穴・検証コマンドを網羅している。
...
└── tests/                   ビルトインプリセットは必須
    ...
    │   └── test.js
    └── analyzers.js         テスト専用ヘルパー (sdd-forge 内部 import を許可)
```
...
`.sdd-forge/overrides.json` に置くプロジェクト全体で 1 つの辞書ファイル。DataSource が返すエントリの説明文を手動で確定したい場合に使う (enrich の AI 生成結果より優先される)。
...
`src/presets/**/data/*.js` および `.sdd-forge/presets/**/data/*.js` の **default export は `register(container)` 形式のファクトリ関数** でなければならない。class 直接 export は許可しない。class を直接 export すると loader はファクトリとして呼び出すため、`new X()` エラーで失敗する。
...
sdd-forge の基底クラス・ユーティリティは **すべて Container 経由で取得** する。データソース実装のファイル先頭では Node.js 組み込みモジュール (`fs`, `path`, `url`, `crypto`) のみ import でき、sdd-forge 内部への相対 import・bare specifier import は禁止する。
...
1. sdd-forge 内部への相対 import (`../../../docs/lib/...`, `../../lib/...`, `../../<sibling-preset>/...`) は禁止。
2. bare specifier (`sdd-forge/api`, `sdd-forge/presets/*` 等) は存在しない (spec 191 で削除済み)。
3. sdd-forge のすべての依存は `register(container)` 関数の内部で `container.get(...)` / `container.getPreset(...)` で取得する。

テスト専用ヘルパー (sdd-forge 内部 import が必要なもの) は `tests/analyzers.js` に分離する (後述 §13.4)。
...
| `paths` | object | 各種パス (srcRoot, sddDir, outputDir, agentWorkDir, logDir, configPath) |
| `config` | object | `.sdd-forge/config.json` の読み込み結果 (未初期化時は null) |
| `lang` | string | `config.lang` (ドキュメント言語) |
| `i18n` | function | 翻訳関数 |
| `logger` | Logger | ログ出力 |
| `agent` | Agent | AI エージェント呼び出し |
| `flowManager` | FlowManager | SDD フロー状態管理 |
...
npm パッケージとして配布する外部 preset は、`package.json` の `peerDependencies` のみで sdd-forge との互換性を表現する。Container API に独立した version フィールドは設けない。
...
- `dependencies` に sdd-forge を入れないこと (本体と重複インストールされ解決が壊れる)。
...
3. **`sdd-forge docs scan --dry-run` で scan パターン検証**
4. **テンプレートを配置** — まず `{{text}}` だけで骨格を作る
5. **DataSource を 1 つずつ実装** — `register(container)` 形式。追加ごとに `sdd-forge docs scan` を実行し `<category>.entries.length` を確認
...
```bash
sdd-forge docs scan --dry-run         # カテゴリ別エントリ件数 summary
sdd-forge docs scan --stdout          # 全 analysis JSON を標準出力
sdd-forge docs scan                   # 本実行
sdd-forge docs build                  # 全パイプライン
npm test                              # 整合性テスト
...
`data/*.js` では sdd-forge 内部への import が禁止されている (§6)。ユニットテストで「Source の parse が特定の AST を返すこと」のような個別検証を書きたい場合、テスト専用ヘルパーを `tests/analyzers.js` に置き、そこからは自由に sdd-forge 内部モジュールを import してよい。テストファイル (`tests/unit/*.test.js`) はこのヘルパーだけを import し、Source 本体から内部モジュール依存を切り離す。
...
`sdd-forge docs init` の `[init] ERROR: N 件のファイルが docs/ に既に存在します` は **failure ではなく情報メッセージ** (`--force` 案内)。exit code で判定する。
...
- `package.json` は不要 (loader は sdd-forge 本体の解決コンテキストを使う)。
...
10. [ ] 各 DataSource 追加後に `sdd-forge docs scan` → `analysis.json` 確認
11. [ ] `analysis.X` を読むなら `X` を書く scan がチェーン内にあるか確認
12. [ ] テンプレートの `{{text}}` を段階的に `{{data}}` に差し替え
13. [ ] `sdd-forge docs build` で全パイプライン完走
14. [ ] ビルトインなら `tests/` 整備 → `npm test` で整合性 PASS
15. [ ] 外部配布時は `package.json` の `peerDependencies` に `sdd-forge` を宣言
