 ...
 *
 * senrail flow review — code quality review after implementation.
 ...
 *  These are auto-generated by senrail pipelines and overwritten on build. */
const REVIEW_EXCLUDE_PATHS = ["docs/", "README.md", "AGENTS.md", `${PRODUCT.managedPath("output")}/`];
...
function classifyReviewCommandError(err, phase) {
  if (err instanceof AgentFailure) {
    ...
    return ReviewFailure.fromAgentFailure({
      ...
      recoveryCommand: resolvedPhase === "impl"
        ...
        : `senrail flow run review --phase ${resolvedPhase}`,
    });
  }
  ...
  const recoveryCommand = phase ? `senrail flow run review --phase ${phase}` : "senrail flow run review";
...
async function runReview(rawArgs) {
  ...
  if (cli.help) {
    ...
    console.log([
      "Usage: senrail flow review [options]",
      "",
      "Options:",
