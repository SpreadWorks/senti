...
export const FLOW_COMMANDS = {
  ...
  get: {
    ...
    direct: {
      helpKey: "flow.get.direct",
      requiresFlow: false,
      explicitTargetResolution: true,
      deferTargetNotFound: true,
      runtimeLog: { stepMetadata: false, writeWhenNoFlow: false },
      command: () => import("./lib/get-direct.js"),
      args: { flags: FLOW_TARGET_GUARD_FLAGS, options: FLOW_TARGET_GUARD_OPTIONS },
      help: [
        `Usage: senti flow get direct ${FLOW_TARGET_GUARD_USAGE}`,
        "",
        "Return the CLI-validated direct-mode state and UserActionPrompt.",
        "The query never selects direct mode and never mutates Flow or Git state.",
        "Exact target guards are required when more than one Flow is active.",
        ...FLOW_TARGET_GUARD_HELP_LINES,
      ].join("\n"),
    },
    "next-action": {
  ...
  run: {
    direct: {
      ...
      args: {
        flags: FLOW_TARGET_GUARD_FLAGS,
        options: withTargetGuardOptions([
          "--action",
          "--reason",
          "--scope",
          "--source",
      ...
      help: [
        `Usage: senti flow run direct --action <actionId> ${FLOW_TARGET_GUARD_USAGE}`,
        "",
        "Execute only an action ID returned by `senti flow get direct` or",
        "`senti flow get next-action`. Direct progress is owned exclusively by",
        "DirectFlowSession; normal post-impl steps are not resumed or credited.",
        "",
        "Options:",
        ...
        "  --scope <paths>       Comma-separated repository-relative direct change scope.",
        "  --source <manual>      Record explicit user-selection provenance.",
        "  --test-command <cmd>  Deterministic bounded verification command.",
        "  --timeout-ms <ms>     Verification timeout (maximum 900000).",
        "  --agent-work-dir <path>  Override the command work directory.",
