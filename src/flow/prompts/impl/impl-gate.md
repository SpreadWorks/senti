   ...
   - Non-semantic failures such as schema, malformed artifact, failed command, no-progress guard, failed test evidence, or missing mechanical evidence are not deferred. Recover them with changed evidence and a retry reset before re-running the gate.
