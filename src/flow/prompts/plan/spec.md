   ...
   - **Before writing spec**:
     ...
     - **Context gathering (supplement-first):** Only read additional context when draft + issue are insufficient.
       - If specific target files are unclear: `senrail flow get context --search "<request text or issue title>" --raw`.
       - If project structure is still unclear: `senrail flow get context <path> --raw` for specific files; `senrail flow get context --raw` only as a last resort.
     - If guardrail articles for spec have NOT been loaded in this session: `senrail flow get guardrail spec`. If output is non-empty, follow these principles. Skip if already present in context.
