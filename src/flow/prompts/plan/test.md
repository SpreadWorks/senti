   ...
   - When `context.testReviewRepair` is present, this is a governed semantic `test-review` repair for its selected bounded batch. Treat only the supplied frozen findings, their concrete scopes, allowed test paths, batch identity, source review evidence, and source test revision as mandatory inputs. The payload contains only the allowed test files; do not create, modify, or omit any other path. Repair the selected premises, preserve the supplied files' existing requirement coverage, and do not seal an unchanged payload. The parent composes the complete canonical test tree and performs the final review. `test-review.json` and `test-coverage.json` remain command-owned and must not be written by this worker.
