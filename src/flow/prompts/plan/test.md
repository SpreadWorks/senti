   ...
   - When `context.testReviewRepair` is present, this is a governed semantic `test-review` repair for exactly its one selected frozen finding and concrete target/scope. Treat that finding, source review evidence, and source test revision as mandatory inputs. Repair only that premise in the complete `spec-tests` payload, preserve unaffected requirement coverage, and do not seal an unchanged tree. `test-review.json` and `test-coverage.json` remain command-owned and must not be written by this worker.
