   ...
   - If the guarded canonical input is missing, invalid, or does not match the current phase, stop without writing an artifact or completing the step. If it is valid and contains no `decision: "apply"` items, write an empty `operations[]`. Never set or rewrite approval: it is owned exclusively by `draft-gate`, which judges an unchanged invalid draft.
