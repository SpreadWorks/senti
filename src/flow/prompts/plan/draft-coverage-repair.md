   ...
   - If the guarded canonical input is missing, invalid, or does not match the current phase, stop without writing an artifact or completing the step. If it is valid and contains no `decision: "apply"` items, write an empty `operations[]`. Never set or rewrite approval. After this worker is done, a Definition-owned parent completion connector may set it only when the canonical review, triage, repair audit, and draft facts are all eligible; `draft-gate` then validates the result read-only.
