...
- Run `senrail flow run review` to perform AI-powered code review scoped to this task's surface.
...
- **If `impl-review.json.verdict` is `REJECTED`**:
  ...
  5. Record repair evidence with `senrail flow set issue-log --step task-review --normalized-finding-id <findingId> --repair-ref-file <path> --task-id <taskId> --reason <text>`. A requirement/guardrail must-fix finding cannot pass `task-gate` without an exact finding id, task scope, and repair reference.
- **If verdict is `PASS` or `ADVISORY`**: Display "レビューの結果、修正の必要はありませんでした。"
- **Retry limit:** Each `senrail flow run review` invocation = 1 attempt (CLI invocation level). The CLI enforces the task-scope `maxAttempts` from `TASK_DEFINITION`.
...
- **Recovery:** Use `review` for review recovery and `gate` for gate recovery: `senrail flow set retry reset <gate|review> <phase> --reason <text> --yes`. The reason is required and audited, one re-evaluation is granted, and unchanged evidence is rejected.
