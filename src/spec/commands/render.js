 ...
 *
 * senrail spec render — render a spec.json into spec.md.
 *
...
function printHelp() {
  console.log(
    [
      "Usage: senrail spec render [options]",
      "",
...
function validatePathOption(raw, name) {
  ...
  if (typeof raw !== "string") {
    process.stderr.write(`senrail spec render: ${name} must be a string\n`);
    process.exit(EXIT_ERROR);
  }
  const trimmed = raw.trim();
  if (!trimmed) {
    process.stderr.write(`senrail spec render: ${name} requires a non-empty path\n`);
    process.exit(EXIT_ERROR);
  }
  if (trimmed.includes("\0")) {
    process.stderr.write(`senrail spec render: ${name} contains invalid NUL byte\n`);
    process.exit(EXIT_ERROR);
  }
  return trimmed;
}
 ...
 * (like spec.md). Re-running `senrail spec render` overwrites.
 *
...
export function renderTaskMarkdown(task) {
  ...
  lines.push("");
  lines.push("<!-- Auto-generated by `senrail spec render` from spec.json.tasks[]. Do not edit manually. -->");
  lines.push("");
  lines.push("## Goal");
...
async function runSpecRender(rawArgs, container) {
  ...
  if (!fs.existsSync(specDir) || !fs.statSync(specDir).isDirectory()) {
    process.stderr.write(`senrail spec render: spec directory not found: ${specDir}\n`);
    process.exit(EXIT_ERROR);
  }

  const specJsonPath = path.join(specDir, "spec.json");
  if (!fs.existsSync(specJsonPath)) {
    process.stderr.write(`senrail spec render: spec.json not found at ${specJsonPath}\n`);
    process.exit(EXIT_ERROR);
  }
  ...
  if (errors.length > 0) {
    process.stderr.write(`senrail spec render: spec.json failed schema validation:\n`);
