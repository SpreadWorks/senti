...
- **Once `sdd-forge flow run finalize-cleanup` completes, both release conditions flip together: the worktree directory is removed and `flow get status` reports `active: false`.** Cleanup itself emits the finalize Report inline in the response envelope (`data.report.text`); subsequent `sdd-forge` commands run from the main repository because the worktree no longer exists.
