...
1. Read guarded `sennel flow get status` and `sennel flow get next-action` for
   the exact active target.
2. Run guarded `sennel flow run direct`. Supply `--record-id <id>` only when
...
3. When the result is `transition-applied`, immediately continue through the
   ordinary `sennel flow get next-action` and normal dispatcher route. The
