...
Use this skill when context has been lost (e.g. after compaction) and you need to resume an in-progress Spec-Driven Development flow.
...
4. Tell the user the exact step to resume from, and the skill to invoke:
   - Mainline phases (`plan` / `impl` / `finalize`) → run `/senti.flow` (the consolidated skill inspects state and resumes from the correct step automatically).
