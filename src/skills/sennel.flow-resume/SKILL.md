...
1. Run `sennel flow resume` and read the output.
   - If it reports "no active flow", tell the user there is no flow to resume and stop.
   - If it reports "multiple active flows", ask the user which spec to resume and re-run with `sennel flow resume --spec <specId>`.
