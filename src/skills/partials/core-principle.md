...
Before presenting any choice to the user, you MUST run `senrail flow get status` and display the `autoApprove` field value. This is not optional — skipping this check is a protocol violation.
- If the current flow `runId` is known, prefer `senrail flow get status <runId>` so the check reads the target flow instead of an unrelated current context.
...
- When starting a new flow while another flow is active, record the `runId` returned by `senrail flow set init`; before and after `senrail flow prepare --run-id <runId>`, verify the target with `senrail flow get status <runId> --expect-run-id <runId>` plus every known `--expect-issue` and `--expect-spec` guard.
...
- If the user explicitly continues an existing flow and the target Issue is known, run `senrail flow get status <runId> --expect-run-id <runId> --expect-issue <n>` when `runId` is known. Without a target `runId`, use bare status for display and do not treat another active flow as authorization to continue it.
- If the user explicitly continues an existing spec target, run `senrail flow get status --expect-spec <spec>` before dispatcher actions.
- If the user explicitly continues an existing runId target for dispatcher continuation, run `senrail flow get status <runId> --expect-run-id <runId>` before dispatcher actions.
...
- Store runId and binding values returned by the CLI as opaque tokens. Never
  ...
  prose.
- A preparing flow still reports `autoApprove: false` in status; use the `senrail flow set auto on --run-id <runId>` response and `senrail flow prepare --run-id <runId>` inheritance for prelude auto mode.
- Bare `senrail flow get status` remains valid for current-context display and for detecting whether any active flow exists before a runId is known.
...
- When a flow command needs an agent/tmp/log base directory for the current invocation, pass `--agent-work-dir <path>` to `senrail flow run ...`.
- Flow commands automatically append stdout/stderr to `.tmp/logs/<flowId>.log`, or `.tmp/logs/no-flow.log` when no flow is active.
- Use `senrail flow get runtime-log` to inspect the latest flow command output after failures.
- Do not wrap flow commands with environment-variable prefixes or shell redirection just to capture logs; keep the command prefix as `senrail flow ...` so approval-prefix rules can match it.
