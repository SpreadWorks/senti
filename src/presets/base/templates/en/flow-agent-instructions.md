...
This project uses Spec-Driven Development powered by senrail.
...
- If skills are unavailable, create a preparing flow with `senrail flow set init --request "<request>"`. Use the returned runId in every subsequent `senrail flow prepare ... --run-id <runId>` and `senrail flow run dispatch --expect-run-id <runId>` command, and consult each command's `--help` for details.
...
- **MUST: Never `cd` out of the worktree path.** The only legitimate exit is after `senrail flow run finalize-cleanup` cleanup completes (the finalize skill announces that transition).
...
Before starting work, run `senrail check freshness`.
When the result is `stale` or `never-built`, suggest running `senrail docs build` to the user. When it is `indeterminate`, report the limiting condition and do not treat docs as fresh.

### Development Workflow

- After modifying skill or preset artifacts in `src/skills/` or `src/presets/`, run `senrail upgrade` to refresh project skills, the SDD sections in AGENTS/CLAUDE, and preset copies.
- Run `senrail docs build` when docs template changes must be reflected in generated documentation.
