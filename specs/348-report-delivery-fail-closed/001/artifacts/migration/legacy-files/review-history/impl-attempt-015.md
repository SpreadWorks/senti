...
### 9. 1. Remove side-effect test import
...
### 10. 1. Extract Repeated Issue Log Path Helpers
...
### 11. 2. Use `try/finally` For All Environment Mutations
...
### 12. 3. Consolidate Fake `gh` Installer Behavior
...
### 13. 4. Bound Recursive Step Traversal
...
### 14. 5. Reduce Large Inline Artifact Fixtures
...
### 15. 6. Avoid Recreating `RunReportCommand` Repeatedly In Single Tests
...
### 16. 1. Add an explicit bound for binding source artifacts
...
### 17. 2. Extract duplicate file hashing logic
...
### 18. 3. Validate source paths during binding creation
...
### 19. 1. Use a More Specific Parameter Name
...
### 20. 2. Prefer Nullish Coalescing for Optional Override
...
### 21. 1. Extract Repeated Command Metadata
...
### 22. 2. Reuse Lifecycle Validation Helper
...
### 23. 3. Rename `reject` to Avoid Ambiguity
...
### 24. 4. Avoid Re-reading Invalid Paths During Pending Recovery
...
### 25. 5. Name Transition Table Fields Explicitly
...
### 26. 1. Centralize report path resolution
...
### 27. 2. Reuse delivery-completion helper in the initial delivery path
...
### 28. 3. Simplify pending delivery persistence
...
### 29. 4. Rename `redolog` to match the loaded artifact
...
### 30. 5. Avoid tuple destructuring in `reportSourcePaths()`
...
### 31. 1. Extract the Revalidation Transition Spec
...
### 32. 2. Replace Remaining String Literal Entrypoint
...
### 33. 3. Share Supported Recovery Entrypoints
...
### 34. 1. Extract repeated target-guard command boilerplate
...
### 35. 2. Shorten and clarify the help summary
...
### 36. 1. Extract Recovery Fixture Setup
...
### 37. 2. Centralize Spec Artifact Paths
...
### 38. 1. Extract audited skipped precondition setup
...
### 39. 2. Improve assertion clarity
...
### 40. 1. Define Delivery State Semantics Once
**Finding key:** loop-1e344c076914f69584b9
**Failure mode:** refactor
**File:** specs/348-report-delivery-fail-closed/spec.md
**Requirement:** R3
**Issue:** **File:** `specs/348-report-delivery-fail-closed/spec.md`
**Requirement:** R3
**Issue:** Both `spec.md` and `spec.json` use `unsent` and `pending` without a shared definition, and `src/flow/lib/run-report.js` persists `pending` in multiple paths. This creates a cross-file contract gap between spec text, machine-readable spec, implementation, and tests.
**Suggestion:** Add identical canonical status definitions to both spec files, then align `run-report.js` and tests to those terms, e.g. `unsent` means no delivery attempt occurred because `gh` was unavailable, while `pending` means an attempted delivery failed and can be retried.
**Suggestion:** **File:** `specs/348-report-delivery-fail-closed/spec.md`
**Requirement:** R3
**Issue:** Both `spec.md` and `spec.json` use `unsent` and `pending` without a shared definition, and `src/flow/lib/run-report.js` persists `pending` in multiple paths. This creates a cross-file contract gap between spec text, machine-readable spec, implementation, and tests.
**Suggestion:** Add identical canonical status definitions to both spec files, then align `run-report.js` and tests to those terms, e.g. `unsent` means no delivery attempt occurred because `gh` was unavailable, while `pending` means an attempted delivery failed and can be retried.
...
### 41. 2. Normalize Issue Log Naming Across Report Flow
**Finding key:** loop-e8156dde68dc1ba4437d
**Failure mode:** refactor
**File:** src/flow/lib/run-report.js
**Requirement:** R7
**Issue:** **File:** `src/flow/lib/run-report.js`
**Requirement:** R7
**Issue:** `run-report.js` uses the variable name `redolog` for data loaded from `issue-log.json`, while specs and tests consistently refer to `issue-log.json` / `issueLog`. This cross-file naming mismatch makes the report source artifact harder to trace.
**Suggestion:** Rename `redolog` to `issueLog` in `run-report.js` and keep persisted/report-facing names aligned with `issueLog`.
**Suggestion:** **File:** `src/flow/lib/run-report.js`
**Requirement:** R7
**Issue:** `run-report.js` uses the variable name `redolog` for data loaded from `issue-log.json`, while specs and tests consistently refer to `issue-log.json` / `issueLog`. This cross-file naming mismatch makes the report source artifact harder to trace.
**Suggestion:** Rename `redolog` to `issueLog` in `run-report.js` and keep persisted/report-facing names aligned with `issueLog`.
...
### 42. 3. Centralize Spec Artifact Path Helpers Across Tests
**Finding key:** loop-9fb9b29ef739d8b12143
**Failure mode:** refactor
**File:** specs/348-report-delivery-fail-closed/tests/report-delivery-fail-closed.test.js
**Requirement:** R1
**Issue:** **File:** `specs/348-report-delivery-fail-closed/tests/report-delivery-fail-closed.test.js`
**Requirement:** R1
**Issue:** Multiple test files independently construct spec-relative artifact paths, including `issue-log.json`, `report.json`, and `specs/${SPEC_ID}` paths. This duplicates layout knowledge across spec tests and unit tests.
**Suggestion:** Add or reuse a small shared helper for spec artifact paths, then use it in both `specs/348-report-delivery-fail-closed/tests/report-delivery-fail-closed.test.js` and `tests/unit/flow/recover-existing-implementation.test.js`.
**Suggestion:** **File:** `specs/348-report-delivery-fail-closed/tests/report-delivery-fail-closed.test.js`
**Requirement:** R1
**Issue:** Multiple test files independently construct spec-relative artifact paths, including `issue-log.json`, `report.json`, and `specs/${SPEC_ID}` paths. This duplicates layout knowledge across spec tests and unit tests.
**Suggestion:** Add or reuse a small shared helper for spec artifact paths, then use it in both `specs/348-report-delivery-fail-closed/tests/report-delivery-fail-closed.test.js` and `tests/unit/flow/recover-existing-implementation.test.js`.
...
### 43. 4. Share Recovery Transition Constants Across Policy And Command
**Finding key:** loop-507f015efd33ada0442b
**Failure mode:** refactor
**File:** src/flow/lib/run-recover-existing-implementation.js
**Requirement:** R2
**Issue:** **File:** `src/flow/lib/run-recover-existing-implementation.js`
**Requirement:** R2
**Issue:** Recovery transition expectations are represented in both `run-recover-existing-implementation.js` and `step-transition-policy.js`, with related entrypoint/status data duplicated in slightly different structures. This creates drift risk between command validation and transition policy.
**Suggestion:** Move the recovery transition metadata into one exported policy-level constant or helper, and have the command consume that instead of maintaining its own positional `expected` table.
**Suggestion:** **File:** `src/flow/lib/run-recover-existing-implementation.js`
**Requirement:** R2
**Issue:** Recovery transition expectations are represented in both `run-recover-existing-implementation.js` and `step-transition-policy.js`, with related entrypoint/status data duplicated in slightly different structures. This creates drift risk between command validation and transition policy.
**Suggestion:** Move the recovery transition metadata into one exported policy-level constant or helper, and have the command consume that instead of maintaining its own positional `expected` table.
...
### 44. 5. Use One Shape For Successful Issue Comment Delivery
**Finding key:** loop-3cc9db3948d776d2dbb7
**Failure mode:** refactor
**File:** src/flow/lib/run-report.js
**Requirement:** R7
**Issue:** **File:** `src/flow/lib/run-report.js`
**Requirement:** R7
**Issue:** Initial delivery and resumed delivery build successful `issueComment` artifacts differently: resumed delivery includes `idempotencyKey`, while initial delivery manually constructs a similar object without the same shape. Tests and report consumers may observe inconsistent delivery metadata for the same final state.
**Suggestion:** Use the existing `completedIssueComment()` helper for both initial and resumed successful delivery paths so the persisted artifact shape is identical.
**Suggestion:** **File:** `src/flow/lib/run-report.js`
**Requirement:** R7
**Issue:** Initial delivery and resumed delivery build successful `issueComment` artifacts differently: resumed delivery includes `idempotencyKey`, while initial delivery manually constructs a similar object without the same shape. Tests and report consumers may observe inconsistent delivery metadata for the same final state.
**Suggestion:** Use the existing `completedIssueComment()` helper for both initial and resumed successful delivery paths so the persisted artifact shape is identical.
