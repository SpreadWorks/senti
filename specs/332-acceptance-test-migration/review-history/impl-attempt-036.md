...
### 28. 2. Improve Unsupported Stage Error Specificity
**Finding key:** loop-ef5dfd521f1d1d63bef0
**Failure mode:** refactor
**File:** src/flow/lib/run-reopen-draft.js
**Requirement:** R8
**Issue:** **File:** `src/flow/lib/run-reopen-draft.js`  
**Requirement:** R8  
**Issue:** The new message says “a supported implementation stage” but does not tell the user which stages are supported. Since the code now depends on `SPEC_CORRECTION_SUPPORTED_STAGES`, the error became less actionable than the old single-stage message.  
**Suggestion:** Include the supported stages in the error message, for example by formatting `SPEC_CORRECTION_SUPPORTED_STAGES.join(", ")`, so the behavior and message stay consistent with the source of truth.
...
### 29. 1. Extract Spec-Correction Rewind Detection
**Finding key:** loop-0b2297986f9acd6f2d01
**Failure mode:** refactor
**File:** src/flow/lib/run-scenario-validity.js
**Requirement:** R4
**Issue:** **File:** `src/flow/lib/run-scenario-validity.js`  
**Requirement:** R4  
**Issue:** The condition `state?.planRewinds?.at(-1)?.category !== "spec-correction"` embeds rewind-history structure and the `"spec-correction"` category string directly in the scenario validity command. This makes the intent harder to scan and risks duplicated string checks if the same exception is needed elsewhere in this file.  
**Suggestion:** Introduce a small local helper such as `isLatestPlanRewindSpecCorrection(state)` and use it when deciding whether to run `listScenarioValidityPreflightFiles`.
...
### 30. 1. Extract Repeated Requirement Lookup
...
### 31. 2. Extract Shared Test Evidence Builders
...
### 33. 4. Avoid Duplicate `fs.existsSync(sourcePath)` Calls
...
### 34. 5. Clarify `input` Parameter Name
...
### 35. 2. Extract repeated readiness evaluation setup
...
### 36. 3. Name the legacy artifact mutation by behavior
...
### 37. 1. Extract a spec fixture helper
...
### 38. 1. Narrow the prompt assertion to the intended rule
...
### 39. 2. Avoid recomputing prompt text in assertions
...
### 40. 4. Ensure Per-Iteration Temp Cleanup
...
### 41. 1. Extract Replay Assertion Helper
...
### 42. 2. Rename Migration Fixture Helper
...
### 43. 3. Avoid Step Array Index Coupling
...
### 44. 1. Add Timeouts To Synchronous Git Calls
...
### 45. 2. Extract Fixture Setup For Readability
...
### 46. 1. Centralize acceptance-review fixture invocation
**Finding key:** loop-67927eef52573145f894
**Failure mode:** refactor
**File:** tests/helpers/acceptance-review-fixture.js
**Requirement:** R8
**Issue:** **File:** `tests/helpers/acceptance-review-fixture.js`  
**Requirement:** R8  
**Issue:** Multiple files propose local helpers for the same repeated `runAcceptanceReviewFixture` call shape: `specs/296-review-gate-defer/tests/retry-exhaustion-defer.test.js` and `specs/310-defer-test-review-exhaustion/tests/test-review-post-hook-deferral.test.js`. Creating separate local helpers risks divergent fixture defaults and naming for the same interface.  
**Suggestion:** Add a shared helper in `tests/helpers/acceptance-review-fixture.js`, such as `runAcceptanceReviewFixtureWithDefaults(fixture, options = {})`, and use it from both spec test files.
**Suggestion:** **File:** `tests/helpers/acceptance-review-fixture.js`  
**Requirement:** R8  
**Issue:** Multiple files propose local helpers for the same repeated `runAcceptanceReviewFixture` call shape: `specs/296-review-gate-defer/tests/retry-exhaustion-defer.test.js` and `specs/310-defer-test-review-exhaustion/tests/test-review-post-hook-deferral.test.js`. Creating separate local helpers risks divergent fixture defaults and naming for the same interface.  
**Suggestion:** Add a shared helper in `tests/helpers/acceptance-review-fixture.js`, such as `runAcceptanceReviewFixtureWithDefaults(fixture, options = {})`, and use it from both spec test files.
...
### 47. 2. Centralize JSON artifact fixture writing
**Finding key:** loop-692265653b9f2d3a0ca3
**Failure mode:** refactor
**File:** tests/helpers/acceptance-review-fixture.js
**Requirement:** R9
**Issue:** **File:** `tests/helpers/acceptance-review-fixture.js`  
**Requirement:** R9  
**Issue:** Several proposals identify duplicated JSON artifact writes across test files, including flow findings, retry source artifacts, and acceptance review artifacts. If each file adds its own writer helper, artifact formatting and default schema behavior can drift.  
**Suggestion:** Provide a shared test helper for plain artifact writes, for example `writeJsonArtifact(dir, file, artifact)`, and keep schema-specific helpers explicitly named separately, such as `writeFlowFindingsArtifactFixture`.
**Suggestion:** **File:** `tests/helpers/acceptance-review-fixture.js`  
**Requirement:** R9  
**Issue:** Several proposals identify duplicated JSON artifact writes across test files, including flow findings, retry source artifacts, and acceptance review artifacts. If each file adds its own writer helper, artifact formatting and default schema behavior can drift.  
**Suggestion:** Provide a shared test helper for plain artifact writes, for example `writeJsonArtifact(dir, file, artifact)`, and keep schema-specific helpers explicitly named separately, such as `writeFlowFindingsArtifactFixture`.
...
### 48. 3. Align spec-correction supported-stage source and error text
**Finding key:** loop-e34458264e815731e756
**Failure mode:** refactor
**File:** src/flow/lib/run-reopen-draft.js
**Requirement:** R8
**Issue:** **File:** `src/flow/lib/run-reopen-draft.js`  
**Requirement:** R8  
**Issue:** `src/flow/lib/plan-rewind.js` owns `SPEC_CORRECTION_SUPPORTED_STAGES`, while `run-reopen-draft.js` emits an unsupported-stage error. The summaries show a naming concern in one file and error specificity concern in the other, which points to a cross-file contract risk: user-facing messaging can drift from the stage source of truth.  
**Suggestion:** Import and format `SPEC_CORRECTION_SUPPORTED_STAGES` directly in the unsupported-stage error, and rename the constant to clarify its spec-correction rewind role if it is only intended for that path.
**Suggestion:** **File:** `src/flow/lib/run-reopen-draft.js`  
**Requirement:** R8  
**Issue:** `src/flow/lib/plan-rewind.js` owns `SPEC_CORRECTION_SUPPORTED_STAGES`, while `run-reopen-draft.js` emits an unsupported-stage error. The summaries show a naming concern in one file and error specificity concern in the other, which points to a cross-file contract risk: user-facing messaging can drift from the stage source of truth.  
**Suggestion:** Import and format `SPEC_CORRECTION_SUPPORTED_STAGES` directly in the unsupported-stage error, and rename the constant to clarify its spec-correction rewind role if it is only intended for that path.
...
### 49. 4. Use consistent “raw artifact” naming
**Finding key:** loop-2035f1ac4954bc465af9
**Failure mode:** refactor
**File:** src/flow/lib/run-gate.js
**Requirement:** R3
**Issue:** **File:** `src/flow/lib/run-gate.js`  
**Requirement:** R3  
**Issue:** The review summaries flag stale or ambiguous naming around artifact models in several files: `latestArtifact` stores raw JSON in `run-gate.js`, while other reviewed artifacts are wrapped as `{ artifact, raw }`, and tests also contain stale verdict/review terminology. This creates cross-file ambiguity between raw JSON payloads and domain artifact instances.  
**Suggestion:** Standardize names so raw parsed JSON uses `rawArtifact`/`latestRawArtifact`, while constructed domain objects use `artifact` or class-specific names. Apply the same convention in related tests when renaming stale titles or variables.
**Suggestion:** **File:** `src/flow/lib/run-gate.js`  
**Requirement:** R3  
**Issue:** The review summaries flag stale or ambiguous naming around artifact models in several files: `latestArtifact` stores raw JSON in `run-gate.js`, while other reviewed artifacts are wrapped as `{ artifact, raw }`, and tests also contain stale verdict/review terminology. This creates cross-file ambiguity between raw JSON payloads and domain artifact instances.  
**Suggestion:** Standardize names so raw parsed JSON uses `rawArtifact`/`latestRawArtifact`, while constructed domain objects use `artifact` or class-specific names. Apply the same convention in related tests when renaming stale titles or variables.
...
### 50. 5. Share step lookup helpers instead of indexing flow steps
**Finding key:** loop-0caf08ca2a1e11f95d57
**Failure mode:** refactor
**File:** tests/helpers/acceptance-review-fixture.js
**Requirement:** R5
**Issue:** **File:** `tests/helpers/acceptance-review-fixture.js`  
**Requirement:** R5  
**Issue:** Multiple files manipulate flow-state steps directly or implicitly: `baseFlowState` mutates supplied steps, and `repair-state-identity.test.js` couples assertions to `state.steps[0]` and `state.steps[1]`. These local patterns can become inconsistent with the flow-state interface.  
**Suggestion:** Add shared test helpers such as `findStep(state, name)`, `setStepStatus(state, name, status)`, and `assertStepStatus(state, name, status)`, then replace positional step access and implicit mutation across the affected tests.
**Suggestion:** **File:** `tests/helpers/acceptance-review-fixture.js`  
**Requirement:** R5  
**Issue:** Multiple files manipulate flow-state steps directly or implicitly: `baseFlowState` mutates supplied steps, and `repair-state-identity.test.js` couples assertions to `state.steps[0]` and `state.steps[1]`. These local patterns can become inconsistent with the flow-state interface.  
**Suggestion:** Add shared test helpers such as `findStep(state, name)`, `setStepStatus(state, name, status)`, and `assertStepStatus(state, name, status)`, then replace positional step access and implicit mutation across the affected tests.
