...
### 11. 1. Extract Cached Command Execution Helper
**Finding key:** loop-cf0e3c970822dc34cc61
**Failure mode:** refactor
...
**Requirement:** R9  
**Issue:** `executeHistoricalFile` and `executeProjectRegression` duplicate the same cache-and-`spawnSync` pattern, differing only by cache key, arguments, and buffer size.  
**Suggestion:** Add a helper such as `executeCached(cacheKey, args, { maxBuffer })` and have both functions call it. This keeps timeout/env/cwd behavior centralized and reduces future drift.
...
### 12. 2. Replace Unused Map Labels With Plain Array
**Finding key:** loop-6486e1d5450ee337938b
**Failure mode:** refactor
**File:** specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js
**Requirement:** R10
**Issue:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R10  
**Issue:** `runtimeRepairTestFiles` is a `Map`, but only `.values()` is used. The descriptive keys are dead metadata at runtime and make the structure look more meaningful than it is.  
**Suggestion:** Change it to a plain array of paths, or use the labels in assertion messages if they are intended to aid diagnosis.
**Suggestion:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R10  
**Issue:** `runtimeRepairTestFiles` is a `Map`, but only `.values()` is used. The descriptive keys are dead metadata at runtime and make the structure look more meaningful than it is.  
**Suggestion:** Change it to a plain array of paths, or use the labels in assertion messages if they are intended to aid diagnosis.
...
### 13. 3. Centralize Acceptance Context Creation
**Finding key:** loop-08bdf213e59e93d7d8fa
**Failure mode:** refactor
**File:** specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js
**Requirement:** R9
**Issue:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R9  
**Issue:** Many tests repeat `acceptance.buildAcceptanceReviewContext({ root: fixture.root, state: fixture.state, diff: fixture.diff })`. This adds noise and makes the tests harder to scan.  
**Suggestion:** Add a local helper like `buildFixtureAcceptanceContext(fixture)` and use it across R3-R8. The helper name also documents that these tests exercise the fixture’s current flow state.
**Suggestion:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R9  
**Issue:** Many tests repeat `acceptance.buildAcceptanceReviewContext({ root: fixture.root, state: fixture.state, diff: fixture.diff })`. This adds noise and makes the tests harder to scan.  
**Suggestion:** Add a local helper like `buildFixtureAcceptanceContext(fixture)` and use it across R3-R8. The helper name also documents that these tests exercise the fixture’s current flow state.
...
### 14. 4. Extract Deferred Finding Disposition Construction
**Finding key:** loop-419a7142bc09572433d3
**Failure mode:** refactor
**File:** specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js
**Requirement:** R9
**Issue:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R9  
**Issue:** Several tests repeat the same object shape for deferred finding dispositions, including the same evidence reference format.  
**Suggestion:** Add a helper such as `dispositionForFinding(finding, finalDisposition)` returning `{ findingId, finalDisposition, evidenceRefs: [...] }`. This removes duplicated string interpolation and makes disposition-specific assertions clearer.
**Suggestion:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R9  
**Issue:** Several tests repeat the same object shape for deferred finding dispositions, including the same evidence reference format.  
**Suggestion:** Add a helper such as `dispositionForFinding(finding, finalDisposition)` returning `{ findingId, finalDisposition, evidenceRefs: [...] }`. This removes duplicated string interpolation and makes disposition-specific assertions clearer.
...
### 15. 5. Simplify Repeated JSON Artifact Writes
**Finding key:** loop-9d67f9f130581130354c
**Failure mode:** refactor
**File:** specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js
**Requirement:** R10
**Issue:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R10  
**Issue:** R10 repeatedly writes JSON artifacts with ``fs.writeFileSync(path, `${JSON.stringify(value)}\n`)``. The repeated serialization detail distracts from the test scenarios.  
**Suggestion:** Add a test-local `writeJsonLine(file, value)` helper and use it for `testResultPath` and `deltaPath` mutations.
**Suggestion:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R10  
**Issue:** R10 repeatedly writes JSON artifacts with ``fs.writeFileSync(path, `${JSON.stringify(value)}\n`)``. The repeated serialization detail distracts from the test scenarios.  
**Suggestion:** Add a test-local `writeJsonLine(file, value)` helper and use it for `testResultPath` and `deltaPath` mutations.
...
### 16. 6. Rename `currentManifest` To Reflect Its Actual Type
**Finding key:** loop-53a7b5f44ea7b0940256
**Failure mode:** refactor
**File:** src/flow/lib/impl-repair-artifacts.js
**Requirement:** R10
**Issue:** **File:** `src/flow/lib/impl-repair-artifacts.js`  
**Requirement:** R10  
**Issue:** The new code assigns `observedCurrent = buildRepairFingerprint(...)` and passes it through the existing `currentManifest` parameter. In `RepairMigrationEvidenceReplacement`, `currentManifest.entries` and `.hash` are used, but the name now obscures that this is the current repair fingerprint manifest/evidence snapshot.  
**Suggestion:** Rename the parameter/member to `currentFingerprint` or `currentFingerprintManifest` consistently in `commitRepairStateMigration`, `RepairMigrationEvidenceReplacement`, and related call sites.
...
### 17. 7. Remove Unused Failure State Or Use It In Diagnostics
**Finding key:** loop-82fdebb94b353426c725
**Failure mode:** refactor
**File:** src/flow/lib/impl-repair-artifacts.js
**Requirement:** R10
**Issue:** **File:** `src/flow/lib/impl-repair-artifacts.js`  
**Requirement:** R10  
**Issue:** `MigrationEvidenceInspection.failure` is stored and frozen, but all callers immediately read only `.current`. The captured failure message is therefore dead state after the warning is emitted.  
**Suggestion:** Either remove the `failure` property from `MigrationEvidenceInspection`, or return/use it in higher-level diagnostics where stale migration evidence is reported.
**Suggestion:** **File:** `src/flow/lib/impl-repair-artifacts.js`  
**Requirement:** R10  
**Issue:** `MigrationEvidenceInspection.failure` is stored and frozen, but all callers immediately read only `.current`. The captured failure message is therefore dead state after the warning is emitted.  
**Suggestion:** Either remove the `failure` property from `MigrationEvidenceInspection`, or return/use it in higher-level diagnostics where stale migration evidence is reported.
...
### 18. 8. Extract Current-Hash Check Helper
**Finding key:** loop-fbf4d9764f76a21bced5
**Failure mode:** refactor
**File:** src/flow/lib/impl-repair-artifacts.js
**Requirement:** R10
**Issue:** **File:** `src/flow/lib/impl-repair-artifacts.js`  
**Requirement:** R10  
**Issue:** Current fingerprint checks are repeated across `isFreshMigrationArtifact`, `hasCurrentRepairDelta`, and `RepairMigrationEvidenceReplacement` using slightly different expressions around `entries.at(-1)?.currentHash === currentFingerprint.hash`.  
**Suggestion:** Add a small helper such as `latestLedgerEntryMatchesFingerprint(ledger, currentFingerprint)` or `latestRepairHash(specDir)` to centralize the “latest ledger entry currentHash” contract.
**Suggestion:** **File:** `src/flow/lib/impl-repair-artifacts.js`  
**Requirement:** R10  
**Issue:** Current fingerprint checks are repeated across `isFreshMigrationArtifact`, `hasCurrentRepairDelta`, and `RepairMigrationEvidenceReplacement` using slightly different expressions around `entries.at(-1)?.currentHash === currentFingerprint.hash`.  
**Suggestion:** Add a small helper such as `latestLedgerEntryMatchesFingerprint(ledger, currentFingerprint)` or `latestRepairHash(specDir)` to centralize the “latest ledger entry currentHash” contract.
...
### 19. 9. Bound Recursive Associated Evidence Resolution
**Finding key:** loop-4e1c2a9f4531d0f8e221
**Failure mode:** refactor
**File:** src/flow/lib/impl-repair-artifacts.js
**Requirement:** R10
**Issue:** **File:** `src/flow/lib/impl-repair-artifacts.js`  
**Requirement:** R10  
**Issue:** `isFreshMigrationArtifact` recursively follows `associatedPrimaryEvidencePath(relativePath)` without an explicit depth or visited-path bound. The current mapping may be acyclic, but the guardrail requires recursive processing to have explicit bounds.  
**Suggestion:** Replace recursion with a loop that tracks visited paths or caps traversal depth, for example `for (let depth = 0; depth < MAX_ASSOCIATED_EVIDENCE_DEPTH; depth++)`, returning false on cycles or limit exhaustion.
**Suggestion:** **File:** `src/flow/lib/impl-repair-artifacts.js`  
**Requirement:** R10  
**Issue:** `isFreshMigrationArtifact` recursively follows `associatedPrimaryEvidencePath(relativePath)` without an explicit depth or visited-path bound. The current mapping may be acyclic, but the guardrail requires recursive processing to have explicit bounds.  
**Suggestion:** Replace recursion with a loop that tracks visited paths or caps traversal depth, for example `for (let depth = 0; depth < MAX_ASSOCIATED_EVIDENCE_DEPTH; depth++)`, returning false on cycles or limit exhaustion.
...
### 30. 1. Extract repeated fixture evidence builders
...
### 31. 2. Avoid repeated requirement lookup
...
### 32. 3. Remove dead `rawEndLine` check
...
### 33. 4. Bound recursive test discovery
...
### 34. 5. Name `omitArtifacts` by behavior
...
### 35. 6. Extract repeated readiness evaluation setup
...
### 36. 1. Ensure per-stage temp directory cleanup is exception-safe
...
### 37. 2. Extract repeated repair evidence paths
...
### 38. 3. Rename migration helper to describe its side effects
...
### 39. 4. Factor repeated post-migration state setup
...
### 40. 1. Add Timeouts To Synchronous Git Calls
...
### 41. 2. Extract Fixture Setup For Readability
...
### 42. 1. Consolidate Acceptance Fixture Runner Helpers
**Finding key:** loop-670b924e156936174cc1
**Failure mode:** refactor
**File:** specs/295-producer-artifact-contract/tests/producer-artifact-contract.test.js
**Requirement:** R5
**Issue:** **File:** `specs/295-producer-artifact-contract/tests/producer-artifact-contract.test.js`  
**Requirement:** R5  
**Issue:** Multiple files propose local wrappers around `runAcceptanceReviewFixture` with the same fixture fields (`root`, `state`, `diff`, `requirementJudgments`). This duplicated helper pattern appears in specs 295, 296, and 332, so file-local helpers may still drift.  
**Suggestion:** Add a shared helper in `tests/helpers/acceptance-review-fixture.js`, such as `fixture.runAcceptanceReview(overrides)`, and update the affected tests to use one interface.
**Suggestion:** **File:** `specs/295-producer-artifact-contract/tests/producer-artifact-contract.test.js`  
**Requirement:** R5  
**Issue:** Multiple files propose local wrappers around `runAcceptanceReviewFixture` with the same fixture fields (`root`, `state`, `diff`, `requirementJudgments`). This duplicated helper pattern appears in specs 295, 296, and 332, so file-local helpers may still drift.  
**Suggestion:** Add a shared helper in `tests/helpers/acceptance-review-fixture.js`, such as `fixture.runAcceptanceReview(overrides)`, and update the affected tests to use one interface.
...
### 43. 2. Standardize Adopted Acceptance Fixture Cleanup
**Finding key:** loop-b52e124cded71aef9df7
**Failure mode:** refactor
**File:** tests/helpers/acceptance-review-fixture.js
**Requirement:** R2
**Issue:** **File:** `tests/helpers/acceptance-review-fixture.js`  
**Requirement:** R2  
**Issue:** Several tests repeat or propose extracting the same `adoptAcceptanceReviewFixture` plus `try/finally cleanup` pattern across spec 296 and spec 310. Keeping this as separate file-local helpers creates duplicate cleanup contracts.  
**Suggestion:** Provide a shared helper such as `withAdoptedAcceptanceReviewFixture(producerFixture, fn)` from the fixture helper module, and use it across acceptance-related tests.
**Suggestion:** **File:** `tests/helpers/acceptance-review-fixture.js`  
**Requirement:** R2  
**Issue:** Several tests repeat or propose extracting the same `adoptAcceptanceReviewFixture` plus `try/finally cleanup` pattern across spec 296 and spec 310. Keeping this as separate file-local helpers creates duplicate cleanup contracts.  
**Suggestion:** Provide a shared helper such as `withAdoptedAcceptanceReviewFixture(producerFixture, fn)` from the fixture helper module, and use it across acceptance-related tests.
...
### 44. 3. Align Review Target Fingerprint Naming Across Modules
**Finding key:** loop-9cfa5a5433d56e20ee14
**Failure mode:** refactor
**File:** src/flow/lib/review-evidence-store.js
**Requirement:** R3
**Issue:** **File:** `src/flow/lib/review-evidence-store.js`  
**Requirement:** R3  
**Issue:** `resolveCurrentReviewTreeSha()` may now return a synthetic tree-plus-diff fingerprint, while related review/gate code still uses names like `treeSha`, `nextTreeSha`, and resolver names implying literal Git tree SHAs. This is an interface naming inconsistency across review evidence and convergence/gate modules.  
**Suggestion:** Rename the exported API and cross-file parameters to `resolveCurrentReviewTargetFingerprint`, `reviewTargetFingerprint`, or similar, then update callers consistently.
**Suggestion:** **File:** `src/flow/lib/review-evidence-store.js`  
**Requirement:** R3  
**Issue:** `resolveCurrentReviewTreeSha()` may now return a synthetic tree-plus-diff fingerprint, while related review/gate code still uses names like `treeSha`, `nextTreeSha`, and resolver names implying literal Git tree SHAs. This is an interface naming inconsistency across review evidence and convergence/gate modules.  
**Suggestion:** Rename the exported API and cross-file parameters to `resolveCurrentReviewTargetFingerprint`, `reviewTargetFingerprint`, or similar, then update callers consistently.
...
### 45. 4. Centralize Spec-Correction Rewind Constants
**Finding key:** loop-6593e18bbbabf141253a
**Failure mode:** refactor
**File:** src/flow/lib/plan-rewind.js
**Requirement:** R1
**Issue:** **File:** `src/flow/lib/plan-rewind.js`  
**Requirement:** R1  
**Issue:** Spec-correction rewind behavior is represented across files by `SPEC_CORRECTION_SUPPORTED_STAGES`, raw `"spec-correction"` string checks, and related reopen/scenario-validity helpers. This spreads one interface concept across modules with inconsistent naming.  
**Suggestion:** Define shared constants for the spec-correction rewind category and supported stages, then import them in `plan-rewind.js`, `run-reopen-draft.js`, and `run-scenario-validity.js`.
**Suggestion:** **File:** `src/flow/lib/plan-rewind.js`  
**Requirement:** R1  
**Issue:** Spec-correction rewind behavior is represented across files by `SPEC_CORRECTION_SUPPORTED_STAGES`, raw `"spec-correction"` string checks, and related reopen/scenario-validity helpers. This spreads one interface concept across modules with inconsistent naming.  
**Suggestion:** Define shared constants for the spec-correction rewind category and supported stages, then import them in `plan-rewind.js`, `run-reopen-draft.js`, and `run-scenario-validity.js`.
...
### 46. 5. Share Deferred Finding Disposition Builders
**Finding key:** loop-2f5b3893560c802a0df0
**Failure mode:** refactor
**File:** specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js
**Requirement:** R9
**Issue:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R9  
**Issue:** Deferred finding source/disposition object construction is duplicated across spec 293, spec 295/296, spec 310, and spec 332 tests, with repeated fields like `findingId`, `finalDisposition`, `evidenceRefs`, `sourceArtifact`, and `sourceFindingId`. This risks inconsistent fixture contracts between tests.  
**Suggestion:** Add shared fixture helpers for deferred finding sources and dispositions, then use them across deferred-flow acceptance tests instead of creating file-local variants.
**Suggestion:** **File:** `specs/332-acceptance-test-migration/tests/acceptance-test-migration.test.js`  
**Requirement:** R9  
**Issue:** Deferred finding source/disposition object construction is duplicated across spec 293, spec 295/296, spec 310, and spec 332 tests, with repeated fields like `findingId`, `finalDisposition`, `evidenceRefs`, `sourceArtifact`, and `sourceFindingId`. This risks inconsistent fixture contracts between tests.  
**Suggestion:** Add shared fixture helpers for deferred finding sources and dispositions, then use them across deferred-flow acceptance tests instead of creating file-local variants.
