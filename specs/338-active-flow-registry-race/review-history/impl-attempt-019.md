...
### 18. 1. Deduplicate Repair Ledger Loading
**Finding key:** loop-560c9ab91c1b93ff92e8
**Failure mode:** refactor
**File:** src/flow/lib/set-step.js
**Requirement:** R1
**Issue:** **File:** `src/flow/lib/set-step.js`  
**Requirement:** R1  
**Issue:** `readImplRepairLedger(specDir)` and the derived `repaired` set are built independently in both `missingCurrentAppliedFindingIds` and `missingGateObservedFindingIds`. This duplicates logic and risks the two recovery paths drifting.  
**Suggestion:** Extract a small helper such as `repairedFindingIds(specDir)` that returns the `Set` of repaired source finding IDs, and reuse it in both functions.
...
### 19. 2. Replace Nested Try/Catch Fallback With Explicit Strategy Loop
**Finding key:** loop-9edd4f93cd61fe7aafd6
**Failure mode:** refactor
**File:** src/flow/lib/set-step.js
**Requirement:** R4
**Issue:** **File:** `src/flow/lib/set-step.js`  
**Requirement:** R4  
**Issue:** `validateBlockedImplRepairRecovery` uses nested `try/catch` blocks to attempt triage-based recovery and then gate-observed recovery. This makes the fallback behavior harder to scan and mixes control flow with error aggregation.  
**Suggestion:** Use an ordered list of recovery strategies, collect failure messages, and return the first successful result. This would make the recovery design more consistent and easier to extend.
**Suggestion:** **File:** `src/flow/lib/set-step.js`  
**Requirement:** R4  
**Issue:** `validateBlockedImplRepairRecovery` uses nested `try/catch` blocks to attempt triage-based recovery and then gate-observed recovery. This makes the fallback behavior harder to scan and mixes control flow with error aggregation.  
**Suggestion:** Use an ordered list of recovery strategies, collect failure messages, and return the first successful result. This would make the recovery design more consistent and easier to extend.
...
### 20. 3. Add Bounds When Reading Issue Log Entries
**Finding key:** loop-3491af2427d0f127aeeb
**Failure mode:** refactor
**File:** src/flow/lib/set-step.js
**Requirement:** R5
**Issue:** **File:** `src/flow/lib/set-step.js`  
**Requirement:** R5  
**Issue:** `missingGateObservedFindingIds` processes `loadIssueLog(root, state.spec).entries` without an explicit bound. Under the `bounded-resource-usage` guardrail, bulk data processing should have explicit size limits.  
**Suggestion:** Apply a documented maximum number of issue-log entries to inspect, or use an existing bounded issue-log read API if available. If the full log must be scanned by design, add a matched spec acknowledgment rationale for `bounded-resource-usage`.
**Suggestion:** **File:** `src/flow/lib/set-step.js`  
**Requirement:** R5  
**Issue:** `missingGateObservedFindingIds` processes `loadIssueLog(root, state.spec).entries` without an explicit bound. Under the `bounded-resource-usage` guardrail, bulk data processing should have explicit size limits.  
**Suggestion:** Apply a documented maximum number of issue-log entries to inspect, or use an existing bounded issue-log read API if available. If the full log must be scanned by design, add a matched spec acknowledgment rationale for `bounded-resource-usage`.
...
### 21. 4. Improve Naming Around “Missing” Findings
**Finding key:** loop-5444601a256d8ba312c0
**Failure mode:** refactor
**File:** src/flow/lib/set-step.js
**Requirement:** R2
**Issue:** **File:** `src/flow/lib/set-step.js`  
**Requirement:** R2  
**Issue:** `missingCurrentAppliedFindingIds` and `missingGateObservedFindingIds` both return finding IDs that need late repair recovery, but “missing” is ambiguous: missing from the review, missing from the ledger, or missing evidence.  
**Suggestion:** Rename them to clarify intent, for example `unrepairedAppliedFindingIds` and `gateObservedUnrepairedFindingIds`, or return `{ unrepairedFindingIds }` from each helper.
**Suggestion:** **File:** `src/flow/lib/set-step.js`  
**Requirement:** R2  
**Issue:** `missingCurrentAppliedFindingIds` and `missingGateObservedFindingIds` both return finding IDs that need late repair recovery, but “missing” is ambiguous: missing from the review, missing from the ledger, or missing evidence.  
**Suggestion:** Rename them to clarify intent, for example `unrepairedAppliedFindingIds` and `gateObservedUnrepairedFindingIds`, or return `{ unrepairedFindingIds }` from each helper.
...
### 22. 5. Extract Recovery Result Construction
**Finding key:** loop-36bd45cbac566433ca71
**Failure mode:** refactor
**File:** src/flow/lib/set-step.js
**Requirement:** R3
**Issue:** **File:** `src/flow/lib/set-step.js`  
**Requirement:** R3  
**Issue:** The `isBlockedImplRepairRecovery` branch in `execute` contains validation, repair completion, response construction, and error-envelope handling inline. This makes `execute` harder to follow and gives the special recovery path a large footprint inside the general step-setting flow.  
**Suggestion:** Move the branch body into a helper such as `recoverBlockedImplRepair({ ctx, state, id, status })`. Keep `execute` responsible for routing and let the helper own recovery-specific behavior.
**Suggestion:** **File:** `src/flow/lib/set-step.js`  
**Requirement:** R3  
**Issue:** The `isBlockedImplRepairRecovery` branch in `execute` contains validation, repair completion, response construction, and error-envelope handling inline. This makes `execute` harder to follow and gives the special recovery path a large footprint inside the general step-setting flow.  
**Suggestion:** Move the branch body into a helper such as `recoverBlockedImplRepair({ ctx, state, id, status })`. Keep `execute` responsible for routing and let the helper own recovery-specific behavior.
...
### 23. 1. Add an explicit snapshot size bound
...
### 24. 2. Rename the local `snapshot` variable
...
### 25. 3. Simplify entry normalization in `ActiveFlowRegistrySnapshot`
...
### 26. 1. Align exact-target validation
...
### 27. 2. Remove duplicated explicit target resolver
...
### 28. 3. Rename captured mutation wrapper for clarity
...
### 29. 1. Extract ReviewEvidence fixture creation
...
### 30. 2. Align the test request text with the asserted behavior
...
### 31. 3. Avoid hard-coded retry exhaustion values repeated inline
...
### 32. 1. Extract Shared Review Convergence Record Setup
...
### 33. 2. Name the Test Around the Observable Behavior
...
### 34. 1. Rename helper to match returned shape
...
### 35. 2. Avoid custom camelCase keys for step status overrides
...
### 36. 1. Extract repeated repair fixture setup
...
### 37. 2. Extract repeated flow manager status application
...
### 38. 3. Use a more specific finding ID constant name
...
### 39. 4. Avoid nondeterministic timestamp generation in fixture data
...
### 40. 1. Standardize Exact Target Naming and Validation
**Finding key:** loop-6f5e95c116fcf13ecfda
**Failure mode:** refactor
**File:** src/lib/flow-manager.js
**Requirement:** R1
**Issue:** **File:** `src/lib/flow-manager.js`
**Requirement:** R1
**Issue:** Multiple files now discuss exact-target mutation semantics, but naming and validation appear inconsistent across `flow-manager.js` and `acceptance-review-artifacts.js`. `captureExactTarget()` requires a spec, while `mutateExactTarget()` may allow weaker expectations, and acceptance decision code branches on snapshot presence to choose mutation behavior.
**Suggestion:** Introduce one shared exact-target contract in `flow-manager.js`, for example `assertExactFlowTargetWithSpec()`, and have acceptance decision mutation paths call through that same interface rather than locally branching on target mode.
**Suggestion:** **File:** `src/lib/flow-manager.js`
**Requirement:** R1
**Issue:** Multiple files now discuss exact-target mutation semantics, but naming and validation appear inconsistent across `flow-manager.js` and `acceptance-review-artifacts.js`. `captureExactTarget()` requires a spec, while `mutateExactTarget()` may allow weaker expectations, and acceptance decision code branches on snapshot presence to choose mutation behavior.
**Suggestion:** Introduce one shared exact-target contract in `flow-manager.js`, for example `assertExactFlowTargetWithSpec()`, and have acceptance decision mutation paths call through that same interface rather than locally branching on target mode.
...
### 41. 2. Unify Bounded Resource Limits Across Registry, Logs, and Review History
**Finding key:** loop-f9b2bf0041371f038228
**Failure mode:** refactor
**File:** src/lib/active-flow-registry.js
**Requirement:** R5
**Issue:** **File:** `src/lib/active-flow-registry.js`
**Requirement:** R5
**Issue:** Several files introduce or rely on bulk scans without explicit bounds: active-flow registry snapshots, review convergence history, issue-log entries, repair source finding IDs, reset step IDs, and changed paths. Adding separate ad hoc limits in each file risks inconsistent failure behavior and unclear ownership.
**Suggestion:** Define shared bounded-resource constants or helper validators for registry entries, issue-log entries, review convergence records, evidence history entries, repair finding IDs, reset steps, and changed paths. Reuse them from `active-flow-registry.js`, `acceptance-review-artifacts.js`, `impl-repair-artifacts.js`, and `set-step.js`.
**Suggestion:** **File:** `src/lib/active-flow-registry.js`
**Requirement:** R5
**Issue:** Several files introduce or rely on bulk scans without explicit bounds: active-flow registry snapshots, review convergence history, issue-log entries, repair source finding IDs, reset step IDs, and changed paths. Adding separate ad hoc limits in each file risks inconsistent failure behavior and unclear ownership.
**Suggestion:** Define shared bounded-resource constants or helper validators for registry entries, issue-log entries, review convergence records, evidence history entries, repair finding IDs, reset steps, and changed paths. Reuse them from `active-flow-registry.js`, `acceptance-review-artifacts.js`, `impl-repair-artifacts.js`, and `set-step.js`.
...
### 42. 3. Align “Repair Evidence” Terminology Across Source and Tests
**Finding key:** loop-8b1344247c3fe077ac45
**Failure mode:** refactor
**File:** src/flow/lib/impl-repair-artifacts.js
**Requirement:** R2
**Issue:** **File:** `src/flow/lib/impl-repair-artifacts.js`
**Requirement:** R2
**Issue:** Naming around repair evidence differs across files: `WORKFLOW_ARTIFACT_PATH_PREFIXES` in source, “missing repair evidence” naming suggestions in tests, and “gate-observed unrepaired findings” naming in `set-step.js`. These refer to related concepts but use different vocabulary.
**Suggestion:** Pick one domain phrase, such as `repair evidence`, and apply it consistently: rename source constants to `REPAIR_EVIDENCE_EXCLUDED_PATH_PREFIXES`, helper outputs to `unrepaired...FindingIds`, and test constants to scenario-specific repair evidence names.
**Suggestion:** **File:** `src/flow/lib/impl-repair-artifacts.js`
**Requirement:** R2
**Issue:** Naming around repair evidence differs across files: `WORKFLOW_ARTIFACT_PATH_PREFIXES` in source, “missing repair evidence” naming suggestions in tests, and “gate-observed unrepaired findings” naming in `set-step.js`. These refer to related concepts but use different vocabulary.
**Suggestion:** Pick one domain phrase, such as `repair evidence`, and apply it consistently: rename source constants to `REPAIR_EVIDENCE_EXCLUDED_PATH_PREFIXES`, helper outputs to `unrepaired...FindingIds`, and test constants to scenario-specific repair evidence names.
...
### 43. 4. Consolidate Repair Recovery Fixture and Transaction Concepts
**Finding key:** loop-8886ff436b3cf580a769
**Failure mode:** refactor
**File:** src/flow/lib/set-step.js
**Requirement:** R1
**Issue:** **File:** `src/flow/lib/set-step.js`
**Requirement:** R1
**Issue:** Repair recovery behavior is spread across `set-step.js`, `impl-repair-artifacts.js`, and multiple unit tests, with duplicated setup and inline transaction construction. This makes the recovery flow harder to compare across implementation and tests.
**Suggestion:** Extract a source helper for building late repair transactions, and mirror that with test fixture helpers for repaired/unrepaired finding scenarios. Keep naming aligned between source helpers and tests so the test setup maps directly to the production recovery path.
**Suggestion:** **File:** `src/flow/lib/set-step.js`
**Requirement:** R1
**Issue:** Repair recovery behavior is spread across `set-step.js`, `impl-repair-artifacts.js`, and multiple unit tests, with duplicated setup and inline transaction construction. This makes the recovery flow harder to compare across implementation and tests.
**Suggestion:** Extract a source helper for building late repair transactions, and mirror that with test fixture helpers for repaired/unrepaired finding scenarios. Keep naming aligned between source helpers and tests so the test setup maps directly to the production recovery path.
...
### 44. 5. Normalize Review Convergence “Current Record” Naming Across Callers
**Finding key:** loop-1b933160fcb4cc1b1d2c
**Failure mode:** refactor
**File:** src/flow/lib/review-convergence.js
**Requirement:** R2
**Issue:** **File:** `src/flow/lib/review-convergence.js`
**Requirement:** R2
**Issue:** `readCurrent()` performs validation, while other files and tests describe convergence recovery in terms of resolving, resetting, or recovering records. The current method name is weaker than the behavior and does not match surrounding recovery terminology.
**Suggestion:** Rename `readCurrent()` to `validateAndReadCurrent()` or `resolveCurrentRecord()`, then update call sites and tests to use the same “current record” language when describing convergence recovery.
**Suggestion:** **File:** `src/flow/lib/review-convergence.js`
**Requirement:** R2
**Issue:** `readCurrent()` performs validation, while other files and tests describe convergence recovery in terms of resolving, resetting, or recovering records. The current method name is weaker than the behavior and does not match surrounding recovery terminology.
**Suggestion:** Rename `readCurrent()` to `validateAndReadCurrent()` or `resolveCurrentRecord()`, then update call sites and tests to use the same “current record” language when describing convergence recovery.
