...
const failingImplReview = {
  blockingFindings: [{
    findingKey: "r8-implementation-mismatch",
...
function driveImplGateAndRetro(root, specDir, fingerprint, guards) {
  ...
  assert.equal(gate.exitCode, 0, JSON.stringify(gate.envelope));
  ...
  try {
    ...
    const { specDir, requirementIds, promptPath, runId, guards } = setupProject(root, {
      reviewResponses: [
        failingImplReview,
        passingImplReview,
        ...allRequirementIds.map((requirementId) => passingGateResponse([requirementId])),
    ...
    assert.match(failedArtifact.blockingFindings[0].findingId, /^[a-f0-9]{64}$/);
    ...
    const repairEvidence = readJson(path.join(specDir, "issue-log.json")).entries.find((entry) => (
      entry.normalizedFindingId === failedArtifact.blockingFindings[0].findingId
    ));
    assert.ok(repairEvidence);
    assert.equal(repairEvidence.step, "impl-review");
    assert.deepEqual(repairEvidence.repairRef.files, ledger.entries[0].changedPaths);
    assert.equal(repairEvidence.timestamp, ledger.entries[0].createdAt);
