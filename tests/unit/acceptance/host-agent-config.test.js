...
test("acceptance fixture inherits the host worktree agent profile and resolves relative providers from that root", () => {
  ...
  writeJson(hostRoot, ".sennel/config.json", baseConfig({
    default: "codex",
    providers: {
      "worktree/codex": {
        command: "tools/codex",
  ...
  writeJson(hostRoot, ".sennel/config.local.json", {
    agent: {
  ...
  const copied = JSON.parse(fs.readFileSync(path.join(destination, ".sennel", "config.json"), "utf8"));
