...
function temporaryRoot() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "senti-legacy-agent-artifact-"));
}
...
describe("legacy agent-host artifact cleanup", () => {
  ...
  it("removes only the legacy Flow handler and preserves project-owned hooks", () => {
    root = temporaryRoot();
    const handler = path.join(root, ".codex/hooks/senti-flow-final-response-guard.mjs");
    ...
    writeJson(config, {
      hooks: {
        Stop: [{
          hooks: [{
            type: "command",
            command: "node .codex/hooks/senti-flow-final-response-guard.mjs",
          }],
        }],
      },
    });
