...
function makeStore() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "senti-issue-log-store-"));
...
test("IssueLogStore accepts only bounded spec.json and spec.md authorities", () => {
  for (const specFile of ["spec.json", "spec.md"]) {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "senti-issue-log-authority-"));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), "senti-issue-log-outside-"));
