...
describe("repair state identity", () => {
  ...
  it("uses Git policy instead of project layout names and captures staged plus worktree state", () => {
    ...
    assert.ok(paths.has(".sennel/config.json"));
    assert.ok(paths.has(".sennel/config.local.json"));
  ...
  it("treats sparse-checkout omissions as environment state instead of deletions", () => {
    ...
    git("sparse-checkout", "set", "app", "specs", ".sennel");
  ...
  it("cleans only the finalized flow baseline ref and is idempotent", () => {
    ...
    assert.equal(git("rev-parse", "refs/sennel/flows/another-run/baseline"), second.commitOid);
    assert.throws(
  ...
  it("uses exact artifact ownership pathspecs for layout-independent acceptance diffs", () => {
    ...
    assert.equal(registry.owns(".sennel/.active-flow"), true);
    assert.equal(registry.owns(".sennel/.active-flow.other-run"), true);
    assert.equal(registry.owns(".sennel/agent-cache/other-flow.json"), true);
    assert.equal(registry.owns(".sennel/recovery/finalize-cleanup/other.json"), true);
  ...
  it("migrates an active legacy flow transactionally and invalidates downstream evidence", () => {
    ...
    assert.equal(git("rev-parse", "refs/sennel/flows/run-test/baseline"), result.state.repairBaseline.commitOid);
  });
