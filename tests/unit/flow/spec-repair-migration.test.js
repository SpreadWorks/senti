...
describe("spec-repair flow-state migration", () => {
  ...
  it("inserts spec-repair before pending spec gate", () => {
    tmp = createTmpDir();
    ...
    assert.ok(planIds.indexOf("review-spec") < planIds.indexOf("spec-repair"));
    assert.ok(planIds.indexOf("spec-repair") < planIds.indexOf("gate"));
  });

  it("marks spec-repair done when spec gate already started", () => {
    tmp = createTmpDir();
