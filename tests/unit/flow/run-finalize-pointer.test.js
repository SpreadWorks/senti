 ...
 * path to `.sdd-forge/last-finalized-spec` in the main repo.
 */
...
describe("flow finalize — last-finalized-spec pointer (AC4)", () => {
  ...
  it("writes pointer with trailing newline under .sdd-forge/", () => {
    tmp = createTmpDir("sdd-finalize-ptr-");
    const specRel = "specs/042-example-feature/spec.json";

    writeLastFinalizedPointer(tmp, specRel);

    const pointerPath = path.join(tmp, ".sdd-forge", "last-finalized-spec");
    assert.ok(fs.existsSync(pointerPath));
    assert.equal(fs.readFileSync(pointerPath, "utf8"), specRel + "\n");
  });

  it("creates .sdd-forge/ when missing", () => {
    tmp = createTmpDir("sdd-finalize-ptr-mkdir-");
    assert.ok(!fs.existsSync(path.join(tmp, ".sdd-forge")));

    writeLastFinalizedPointer(tmp, "specs/001-x/spec.json");

    assert.ok(fs.existsSync(path.join(tmp, ".sdd-forge", "last-finalized-spec")));
  });

  it("overwrites previous pointer on subsequent finalize runs", () => {
    tmp = createTmpDir("sdd-finalize-ptr-overwrite-");
  ...
  it("is a no-op when targetRoot or specPath is missing", () => {
    tmp = createTmpDir("sdd-finalize-ptr-noop-");
