...
describe("R3: flow set argument validation → structured codes (table-driven)", () => {
  ...
  const CASES = [
    ...
    { name: "set summary with no json", argv: ["flow", "set", "summary"], code: "INVALID_USAGE" },
    { name: "set summary with invalid JSON", argv: ["flow", "set", "summary", "{not-json}"], code: "INVALID_JSON" },
    { name: "set summary with non-array JSON", argv: ["flow", "set", "summary", '{"a":1}'], code: "INVALID_ARG_VALUE" },
    { name: "set test-summary --json with invalid JSON", argv: ["flow", "set", "test-summary", "--json", "{not-json}"], code: "INVALID_JSON" },
    { name: "set test-summary --json with non-object JSON", argv: ["flow", "set", "test-summary", "--json", '"a"'], code: "INVALID_ARG_VALUE" },
