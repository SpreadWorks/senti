...
function createProject() {
  ...
  fs.writeFileSync(path.join(root, ".sennel", "config.local.json"), JSON.stringify({
    plugin: {
...
function attemptArtifacts(root, runId = "run-440") {
  ...
  const worktreePath = path.join(root, ".sennel", "worktree", `feature-${specId}`);
  return {
...
describe("worktree prepare binding transaction", () => {
  it("rejects a second branch prepare before changing the shared checkout", async () => {
    ...
    const registryBefore = fs.readFileSync(path.join(root, ".sennel", ".active-flow"), "utf8");
    ...
    assert.equal(fs.readFileSync(path.join(root, ".sennel", ".active-flow"), "utf8"), registryBefore);
  ...
  it("verifies the persisted exact identity through a fresh worktree manager before publish", async () => {
    ...
    assert.deepEqual(
      JSON.parse(fs.readFileSync(path.join(root, ".sennel", ".active-flow"), "utf8")),
  ...
  it("recovers an exact stale prepare attempt after SIGKILL following git worktree add", async () => {
    ...
    const attempt = JSON.parse(fs.readFileSync(
      path.join(root, ".sennel", ".worktree-prepare-attempt.json"),
      "utf8",
    ));
    ...
    assert.equal(fs.existsSync(path.join(root, ".sennel", ".worktree-prepare-attempt.json")), true);
    ...
    assert.equal(fs.existsSync(path.join(root, ".sennel", ".worktree-prepare-attempt.json")), false);
  });
  ...
  it("publishes explicit null for an Issue-less worktree", async () => {
    ...
    const binding = JSON.parse(fs.readFileSync(path.join(result.worktreePath, ".sennel", "flow-identity.json"), "utf8"));
  ...
  it("recovers the exact transition after SIGKILL immediately after writer lock acquisition", async () => {
    ...
    const specDirectory = path.join(root, "specs", specId);
    const markerPath = path.join(result.worktreePath, ".sennel", "flow-identity.issue-transaction.json");
  ...
  for (const publicationAttack of ["foreign", "third", "mismatched"]) {
    it(`fails stopped for a ${publicationAttack} writer publication authority`, async () => {
      ...
      const authorityPaths = [
        ...
        lockPath,
        markerPath,
        path.join(result.worktreePath, ".sennel", "flow-identity.json"),
  ...
  it("fails stopped when a stale writer lock belongs to another Issue transition", async () => {
    ...
    const lockPath = path.join(specDirectory, ".flow.json.writer.lock");
    const markerPath = path.join(result.worktreePath, ".sennel", "flow-identity.issue-transaction.json");
    const bindingPath = path.join(result.worktreePath, ".sennel", "flow-identity.json");
    const owner = JSON.parse(fs.readFileSync(lockPath, "utf8"));
  ...
  it("recovers the original identity after process interruption during marker publication", async () => {
    ...
    assert.equal(restarted.resolveWorktreeBinding().issue, null);
    assert.equal(
      fs.existsSync(path.join(result.worktreePath, ".sennel", "flow-identity.issue-transaction.json")),
      false,
    );
  });
  ...
  it("ignores only the six exact flow identity authority files", async () => {
    ...
    const similarBinding = path.join(result.worktreePath, ".sennel", "flow-identity.backup.json");
    const similarTransition = `${exactTransition}.backup`;
    const similarReceipt = path.join(result.worktreePath, ".sennel", ".flow-identity.publication.json.backup");
    const similarIntent = path.join(result.worktreePath, ".sennel", ".flow-identity.publication.intent.backup");
    const similarReceiptTemp = path.join(result.worktreePath, ".sennel", ".flow-identity.publication.receipt.tmp.backup");
    const similarBindingTemp = path.join(result.worktreePath, ".sennel", ".flow-identity.publication.binding.tmp.backup");
    ...
    assert.notEqual(git(result.worktreePath, ["check-ignore", ".sennel/flow-identity.json"]), "");
    assert.notEqual(
      git(result.worktreePath, ["check-ignore", ".sennel/flow-identity.issue-transaction.json"]),
      "",
    );
    assert.notEqual(
      git(result.worktreePath, ["check-ignore", ".sennel/.flow-identity.publication.json"]),
      "",
    );
    for (const authority of [
      ".sennel/.flow-identity.publication.intent",
      ".sennel/.flow-identity.publication.receipt.tmp",
      ".sennel/.flow-identity.publication.binding.tmp",
    ]) {
    ...
    assert.match(status, /\.sennel\/flow-identity\.backup\.json/);
    assert.match(status, /\.sennel\/flow-identity\.issue-transaction\.json\.backup/);
    assert.match(status, /\.sennel\/\.flow-identity\.publication\.json\.backup/);
    assert.match(status, /\.sennel\/\.flow-identity\.publication\.intent\.backup/);
    assert.match(status, /\.sennel\/\.flow-identity\.publication\.receipt\.tmp\.backup/);
    assert.match(status, /\.sennel\/\.flow-identity\.publication\.binding\.tmp\.backup/);
  });
  ...
  it("retains PostWorktree, artifact, plugin, docs, Git, and registry behavior", async () => {
    ...
    assert.deepEqual(
      fs.readFileSync(path.join(result.worktreePath, ".sennel", "config.local.json")),
      fs.readFileSync(path.join(root, ".sennel", "config.local.json")),
    );
    const analysis = JSON.parse(fs.readFileSync(
      path.join(result.worktreePath, ".sennel", "output", "analysis.json"),
      "utf8",
    ));
  ...
  it("writes a configured spec root only in the base repository", async () => {
    ...
    fs.writeFileSync(path.join(root, ".sennel", "config.json"), JSON.stringify(config, null, 2));
    git(root, ["add", ".sennel/config.json"]);
  ...
  it("keeps required-config preflight ahead of worktree and binding side effects", async () => {
    const { root, config } = createProject();
    fs.appendFileSync(path.join(root, ".sennel", "config.json"), "\n");
    const ctx = prepareContext(root, config);
    const registryPath = path.join(root, ".sennel", ".active-flow");
    const currentPath = path.join(root, ".sennel", ".current-flow");
