...
describe("flow dispatch CLI", () => {
  ...
  it("keeps the actual approved approval worker contract free of boundary review metadata", () => {
    ...
    const legacyWorkerAction = {
      ...boundaryAction,
      directive: {
        kind: "execute_step",
        terminal: false,
        requiresUserAction: false,
        action: boundaryAction.directive.action,
      },
    };
    ...
    assert.equal(invocation.authorization.approvalToken, legacyIdentity.approvalToken());
    assert.doesNotMatch(prompt, /"actionPrompt"/);
    assert.doesNotMatch(JSON.stringify(invocation), /actionPrompt/);
    assert.deepEqual(resumed.envelope.data.nextAction, boundaryAction);
    assert.deepEqual(
