...
function tmpDir() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-svc-"));
  roots.push(root);
  return root;
}
...
describe("Agent.resolve(commandId)", () => {
  ...
  it("resolves to a profile-routed provider when SENNEL_PROFILE is unset and useProfile is configured", () => {
    const cfg = {
      agent: {
        default: "claude/opus",
