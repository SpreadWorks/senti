...
async function loadRuntime(runtimeRoot, projectRoot) {
  process.env.SENTI_WORK_ROOT = projectRoot;
  const [
    containerModule,
    { coreCommandRegistry },
    { dispatch },
...
export async function runRuntimeEvaluatorScenario({ diagnosticPath, sourceRoot }) {
  ...
  try {
    ...
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), "senti-runtime-evaluator-project-"));
    runtimeV1Root = fs.mkdtempSync(path.join(os.tmpdir(), "senti-runtime-evaluator-v1-"));
    runtimeV2Root = fs.mkdtempSync(path.join(os.tmpdir(), "senti-runtime-evaluator-v2-"));
    ...
    for (const runtimeRoot of [runtimeV1Root, runtimeV2Root]) {
      writeFile(runtimeRoot, "package.json", `${JSON.stringify({
        name: "senti-runtime-evaluator-fixture",
        version: "0.0.0",
        type: "module",
      }, null, 2)}\n`);
    }
    ...
    writeFile(projectRoot, ".senti/config.json", `${JSON.stringify({
      lang: "en",
      type: "base",
